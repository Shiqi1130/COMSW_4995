...
import math
import cv2
def imageresize(img, scale=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False, ds_method='direct'):
    ...
    #print('!!!!!!',sym_len_Hs, sym_len_He, in_H,sym_len_Hs + in_H,sym_len_Hs + in_H+sym_len_He)
    sym_patch = img[:, -sym_len_He:, :]
   # print('??', sym_patch.shape)
    inv_idx = np.arange(sym_patch.shape[1] - 1, -1, -1).astype(np.int64)
    sym_patch_inv = np.take(sym_patch, inv_idx, axis=1)
   # print('!!!',sym_patch_inv.shape,sym_patch.shape)
